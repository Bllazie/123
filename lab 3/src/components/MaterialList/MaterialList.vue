  ...
  <div :class="$style.root">
    ...
    >
      <template v-slot:control="{ item }">
        ...
        <Btn @click="onClickShow(item.id)" theme="info">Список Драгоценностей</Btn>
      </template>
    </Table>
    <router-link :to="{ name: 'MaterialEdit' }">
...
import { selectItems, removeItem, fetchMaterials  } from '@/store/materials/selectors';
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      onClickShow: (id) => {
        router.push({ name: 'EarringsMaterial', query: { materialId: id } });
      },
    };
  },
};
